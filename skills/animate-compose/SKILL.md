...
Build motion in Jetpack Compose: whether it should exist, what it should feel like, and how
it behaves when interrupted or switched off. Every rule this skill applies comes from
...
0. The product's standard — `DECIDED` entries in MOTION.md, or a design system it points to
1. Pragmatic M3 Expressive
2. Craft defaults from `references/standards.md`
3. Ask

Resolution is per decision, not per codebase. A product that customised three things follows
its own standard for those three and M3E for everything else. The degree of customisation
changes nothing.
...
If the project has no `material3` dependency at all — components built on Compose Foundation
and UI — ask once whether M3E motion is the wanted fallback rather than assuming it. That
product declined Material rather than customising it.

## Hard rules

1. Run the sequence in order. Do not start at the spring.
2. No invented values. Every spec resolves to a token symbol — `MaterialTheme.motionScheme`
   or the product's own. A literal `tween(300)` at a call site is a defect (T-002).
...
5. `floor` rules hold regardless of MOTION.md. A product may choose its motion language; it
   may not choose to read frame-rate state during composition.
...
delight. Cannot name one? Do not build it. Background data arriving is not a purpose (T-014).
...
Keep Expressive spatial off dense data (T-004). Exits at roughly half the entrance (T-010).

### 5. Interruption and gesture

Anything the user can start, they can change their mind about (T-012). Gesture-driven values
are `Animatable` with velocity carried into the release (F-002). Back follows the finger
(T-008).
...
carry a `reduced motion` marker (T-006). Hand-driven motion reads the duration scale itself
(F-005).

### 7. Phase check

Frame-rate values read in layout or draw, never composition (F-001). No animated `padding`
on a large subtree (F-008). Depth beyond this: `chrisbanes/skills` → `compose-performance`.
...
Opinionated and short. Recommend one thing. Say plainly when the answer is that this should
not animate.
